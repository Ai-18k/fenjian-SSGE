...
# SPECS：成品规格表，键为规格名，值为 (最小克重, 最大克重)
...
VALID_MIN = 65              # 全局合法重量下限（克）
VALID_MAX = 700             # 全局合法重量上限（克）
DEFAULT_TOTAL = 25000         # 默认批次条数
DEFAULT_OUTSIDE_RATE = 0.01   # 默认超规鱼比例（1%）
...
@dataclass
class FishRecord:
    """单条鱼的批次记录（写入 CSV 的原始数据）。"""

    id: int                      # 鱼序号（1-based）
    weight: int                  # 克重
    spec: str | None = None      # 规格名（超规时为 None）
    outside: bool = False      # 是否超规（相对启用规格）
...
class BatchSummary:
    ...
    total: int = 0                                    # 总条数
    total_weight: int = 0                             # 总重量（克）
    outside_count: int = 0                            # 超规条数
    inside_count: int = 0                               # 规格内条数
    spec_counts: dict[str, int] = field(default_factory=dict)  # 各规格条数
    seed: int = DEFAULT_SEED                          # 随机种子
    enabled_specs: list[str] = field(default_factory=list)     # 启用规格列表
...
def save_csv(records: list[FishRecord], path: Path) -> None:
    """将 FishRecord 列表写入 CSV（id, weight, spec, outside）。"""
