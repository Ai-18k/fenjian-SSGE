...
# 目标盒重区间（克）：4980~5030，中心 5005
TARGET_MIN = 4980   # 盒重下限（克）
TARGET_MAX = 5030   # 盒重上限（克）
TARGET_MID = 5005   # 盒重中心值（克），用于评分

# SPEC_15P：15p 规格的参考参数（566-700g，7/8/9 尾）
SPEC_15P = {
    "range": (566, 700),       # 单尾重量区间（克）
    "counts": (7, 8, 9),       # 合法装箱尾数
    "primary_count": 8,        # 中区锚定尾数（8 尾 ≈ 5kg）
}
...
class BucketRange:
    ...
    small: tuple[int, int]    # 小区间 (lo, hi) 克
    medium: tuple[int, int]   # 中区间 (lo, hi) 克
    large: tuple[int, int]    # 大区间 (lo, hi) 克

    def as_dict(self) -> dict[str, tuple[int, int]]:
        return {"small": self.small, "medium": self.medium, "large": self.large}


# BucketName：区段名称字面量类型
