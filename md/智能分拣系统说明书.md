...
| `BoxPlanner` | 对该规格料道快照做 DFS 自由组合，按 `SPECS[spec]["counts"]` 枚举合法尾数，选最接近 5005 g 且达标的封箱方案 |
...
1. 某规格三区鱼总数 ≥ `min(SPECS[spec]["counts"])` 才尝试（**尾数按规格表，非固定 7–8**）；
2. 将该规格小/中/大料道鱼展平为快照，调用 `plan/深度搜索.py` 的 DFS；
3. 对 `SPECS[spec]["counts"]` 中每个合法尾数枚举子集（**可跳过 FIFO 顺序**），总重须在 [4980, 5030] g；
4. 评分 `|总重 − 5005|`，缺小/中/任一分区方案加惩罚（+1.2）；
5. 选最优方案，按 `pick_ids` 从料道移除对应鱼并记为已装箱。
...
|------|----------|------|
| 15p | 7, 8 | 每盒 7 或 8 尾 |
| 20p | 10, 11 | 每盒 10 或 11 尾 |
| 25p | 12, 13, 14 | 每盒 12～14 尾 |
...
封箱由 `BoxPlanner.find_plan()` 完成：将该规格小/中/大 FIFO 料道中的鱼 **展平为快照**，用 DFS **自由组合**（不必按队头顺序取鱼），小/中/大均可参与或某一区取 0 条。

#### 算法步骤

1. 该规格三区鱼总数 ≥ `min(SPECS[spec]["counts"])` 才尝试；
2. 枚举目标尾数 `count` ∈ `SPECS[spec]["counts"]`（**按规格不同**，如 15p→7/8，20p→10/11，25p→12/13/14）；
3. DFS 在快照中选取恰好 `count` 条鱼的子集，总重须在 **[4980, 5030] g**；
4. 评分 `score = |总重 − 5005|`；若选中鱼缺少任一分区（小/中/大），`score += 1.2`；
5. 取 `score` 最小方案，按 `pick_ids` 从各料道移除对应鱼并封箱。

#### 15p 示例（counts = 7, 8）

| 尾数 | 说明 |
|------|------|
| 7 或 8 | 每盒合法尾数，由 DFS 在快照中选最优子集 |
| 总重 | 4980–5030 g |
...
| 尾数 | 说明 |
|------|------|
| 10 或 11 | 每盒合法尾数，规则同 15p，仅尾数区间不同 |

#### 与旧 FIFO 队头法的区别

- **旧法**：小/中/大各自 FIFO，只能从各区 **队头连续取** 前缀组合；
- **现法**：料道快照内 **任意选鱼** 成盒，缓解滑道有限时的顺序堵死问题。

#### 与动画页的关系

`fifo_monitor.html` 仍使用队头前缀枚举做 **可视化演示**；成盒统计、CSV 报告以 **`Scheduler_Engine.BoxPlanner`（DFS）** 为准。
