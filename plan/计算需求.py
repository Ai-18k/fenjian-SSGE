...
class DemandOption:
    ...
    target_count: int
    remaining_count: int
    remaining_weight: tuple[int, int]
    next_fish_ranges: list[tuple[int, int]]
...
class SpecDemand:
    ...
    current_count: int
    current_weight: int
    options: list[DemandOption] = field(default_factory=list)
    next_fish_ranges: list[tuple[int, int]] = field(default_factory=list)
    complete: bool = False
...
class BoxDemandCalculator:
    ...
    示例：15p 已进 6 条各 570g（总重 3420g）
      - 若目标 7 尾：还需 1 条，剩余重 1560~1610g → 下一条需 566~610g（与规格交集）
      - 若目标 8 尾：还需 2 条，剩余重 1560~1610g → 下一条需 860~1044g 与规格无交集
    """
